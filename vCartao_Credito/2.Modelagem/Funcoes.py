...
def plot_histograma(variavel, df, label=None):
    ...
    if label == None:
        ...
        plt.hist(df[variavel], color='skyblue', edgecolor='black', label=label)
    ...
    elif label == 'Attrition_Flag': 

        plt.hist(df[df['Attrition_Flag'] == 0][variavel],  
                 alpha = 0.5,
                 label=['Existing Customer'])
        
        plt.hist(df[df['Attrition_Flag'] == 1][variavel],  
                 alpha = 0.5,
